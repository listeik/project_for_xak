...
export default function KpiCards({ result }: { result: Calculation }) {
  ...
  return <div className="kpi-grid">
    <article className="kpi-card"><div className="kpi-label">NPV затрат <CircleDollarSign size={17} /></div><div className="kpi-value">{number(kpi.npv, 2)} <span>млн</span></div><p>Постоянные цены 2035 · дисконтировано</p><span className="kpi-foot">Стоимость выданной тонны: {kpi.cost_per_served_ton === null ? "нет выдачи" : `${number(kpi.cost_per_served_ton, 3)} млн/т`}</span></article>
    ...
    <article className="kpi-card"><div className="kpi-label">Инвестиции CAPEX <TrendingUp size={17} /></div><div className="kpi-value">{number(kpi.total_capex, 0)} <span>/ {number(kpi.horizon_capex_limit, 0)}</span></div><div className="capex-track"><span style={{ width: `${Math.min(100, kpi.total_capex / kpi.horizon_capex_limit * 100)}%` }} /></div><span className="kpi-foot">До 2037: {number(kpi.capex_through_2037, 0)} / {number(kpi.capex_limit_2037, 0)} млн · осталось {number(kpi.capex_headroom_2037, 0)} млн</span></article>
