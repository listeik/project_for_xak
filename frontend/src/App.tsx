...
export default function App() {
  ...
  const { plan, snapshot, caseData, snapshotCaseData, pending, busy, error } = planner;
  const [view,setView]=useState<'plan'|'stress'|'compare'|'report'>('plan');
  const [requestedYear, setYear] = useState(2035);
  const year = caseData?.years.includes(requestedYear) ? requestedYear : caseData?.years[0] ?? 2035;
      ...
      <main id="overview">
        ...
        {planner.bootError ? <section className="empty-state panel" role="alert"><Database size={36} /><h2>Расчётный сервер недоступен</h2><p>{planner.bootError}</p><button className="button button-primary" onClick={planner.retryBoot}><RefreshCw size={16} />Подключиться снова</button></section>
          : !caseData || !plan ? <section className="empty-state panel" role="status"><LoaderCircle className="spin" size={32} /><h2>Подключаем расчётный контур</h2><p>Загружаем исходные условия и план с сервера. При первом запуске сервиса это может занять около минуты.</p></section>
            : <>
              <div className="result-status" aria-live="polite"><div><i className={`status-light ${pending || busy ? 'loading' : ''}`} /><span>{busy === 'optimizing' ? 'Оптимизация закупок…' : busy === 'importing' ? 'Проверяем загруженный план…' : pending ? error ? 'Расчёт не обновлён' : 'Пересчитываем план…' : 'Расчёт обновлён'}</span></div>{result && <span>Показан результат: <strong>{result.scenario === 'BASE' ? 'BASE' : 'MANDATORY STRESS'}</strong>{result.demand_profile !== 'BASE' ? ` · спрос ${result.demand_profile}` : ''}{pending ? ' · предыдущая версия плана' : ''}{result.assumptions.research_factors_active ? ' · исследовательский расчёт' : ''}{result.assumptions.research_shock ? ` · ${result.scenario_id}` : ''}</span>}</div>
              {result && <KpiCards result={result} />}
              <nav className="workflow-nav" aria-label="Этап работы">{([['plan','1. Настроить план'],['stress','2. Проверить шок'],['compare','3. Сравнить стратегии'],['report','4. Сохранить решение']] as const).map(([key,title])=><button className={view===key?'active':''} aria-pressed={view===key} key={key} onClick={()=>setView(key)}>{title}</button>)}</nav>
              {view==='plan'&&<div className="workflow-intro"><strong>{plan.contract_lock?'Просмотр плана с принятыми обязательствами':'1. Выберите закупки и инвестиции'}</strong><p>Год переключает заказы и схему поставок. Верхние показатели относятся ко всему горизонту. BASE/STRESS здесь пересчитывает те же решения; подбор дополнительных действий находится на шаге 2.</p>{plan.contract_lock&&<button className="button" onClick={()=>{const lock=plan.contract_lock!;planner.updatePlan({...plan,yearly_orders:lock.baseline_orders as typeof plan.yearly_orders,yearly_reservations:lock.baseline_reservations as typeof plan.yearly_reservations,contract_lock:null,additional_orders:{},research_shock:null,scenario:'BASE',demand_profile:'BASE'});}}>Вернуться к исходному BASE-плану</button>}</div>}
              {snapshot && <AnalyticsBoard plan={snapshot.plan} view={view} updatePlan={(next)=>{planner.updatePlan(next);setView('plan');}} disabled={pending || !!busy} />}
              {view==='plan' && planner.report && <div className="optimization-report" role="status"><Sparkles size={20} /><div><strong>Заказы оптимизированы · инвестиции сохранены</strong><p>NPV: {number(planner.report.npv_before, 2)} → {number(planner.report.npv_after, 2)} млн. {planner.report.savings >= 0 ? 'Снижение затрат' : 'Увеличение затрат'}: {number(Math.abs(planner.report.savings), 2)} млн.</p>{(!planner.report.baseline_feasible || planner.report.baseline_has_shortage) && <p>Исходный план имел дефицит или нарушения ограничений; дефицит составлял {number(planner.report.baseline_shortage_t, 2)} т. Разность стоимости не означает сравнение равноценных стратегий.</p>}<details><summary>Область оптимальности</summary><p>{planner.report.explanation}</p></details></div></div>}
              <div className="main-grid" id="network" hidden={view!=='plan'}><div className="main-visuals">{result && row && snapshotCaseData ? <LogisticsGraph row={row} caseData={snapshotCaseData} year={year} setYear={setYear} /> : <section className="panel empty-state first-calculation"><Fuel size={35} /><h2>Готовим первый расчёт</h2><p>{error ? 'Исправьте параметры или повторите запрос.' : 'Баланс, фактические поставки и стоимость появятся после ответа сервера.'}</p></section>}
              ...
              {result && view==='plan' && <><div className="finance-layout"><FinanceChart result={result} /><section className="panel strategy-note"><p className="eyebrow"><Orbit size={13} /> Операторский контекст</p><h2>Резерв — часть плана</h2><p>{result.assumptions.reserve_target_days} дней годового спроса должны быть обеспечены физическим запасом на начало года. Закупки, инвестиции и сроки доставки проверяются вместе.</p><div className="strategy-metrics"><div><strong>{result.annual_balances.length}</strong><span>лет планирования</span></div><div><strong>{result.assumptions.reserve_target_days}</strong><span>дней резерва</span></div><div><strong>{snapshotCaseData?.sources.length}</strong><span>каналов снабжения</span></div></div><a href="#methodology">Допущения расчёта <ChevronRight size={15} /></a></section></div><ResultTables result={result} year={year} /></>}
            </>}
