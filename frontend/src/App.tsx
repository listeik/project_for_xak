...
export default function App() {
  ...
  return <div className="app-shell">
    <aside className="navigation-rail" aria-label="Навигация"><a href="#overview" className="rail-brand" aria-label="Космоконтур: к началу"><Orbit size={25} /></a><div className="rail-links"><a href="#overview" className="rail-link active" title="Обзор" aria-label="Обзор"><LayoutDashboard size={20} /></a><a href="#network" className="rail-link" title="Сеть снабжения" aria-label="Сеть снабжения"><Network size={20} /></a><a href="#balance" className="rail-link" title="Баланс" aria-label="Таблица баланса"><Database size={20} /></a><a href="#constraints" className="rail-link" title="Ограничения" aria-label="Проверка ограничений"><ShieldCheck size={20} /></a></div><a href="#methodology" className="rail-link rail-bottom" title="Методология" aria-label="Методология"><CircleHelp size={20} /></a><span className="rail-version">v2.0</span></aside>
        ...
        {planner.bootError ? <section className="empty-state panel" role="alert"><Database size={36} /><h2>Расчётный сервер недоступен</h2><p>{planner.bootError}</p><button className="button button-primary" onClick={planner.retryBoot}><RefreshCw size={16} />Подключиться снова</button></section>
          : !caseData || !plan ? <section className="empty-state panel" role="status"><LoaderCircle className="spin" size={32} /><h2>Подключаем расчётный контур</h2><p>Загружаем исходные условия и план с сервера. При первом запуске сервиса это может занять около минуты.</p></section>
            : <>
              ...
              {snapshot && <AnalyticsBoard plan={snapshot.plan} updatePlan={planner.updatePlan} disabled={pending || !!busy} />}
              ...
              <div className="main-grid" id="network"><div className="main-visuals">{result && row && snapshotCaseData ? <LogisticsGraph row={row} caseData={snapshotCaseData} year={year} setYear={setYear} /> : <section className="panel empty-state first-calculation"><Fuel size={35} /><h2>Готовим первый расчёт</h2><p>{error ? 'Исправьте параметры или повторите запрос.' : 'Баланс, фактические поставки и стоимость появятся после ответа сервера.'}</p></section>}
