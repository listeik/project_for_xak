...
## Аналитика и конкурсные материалы 2.2

- **Сравнение стратегий:** Earth / Hybrid / Reserve, BASE и STRESS на единой базе;
  NPV, CAPEX, сервис, дефицит, стоимость выданной тонны.
- **Цена устойчивости:** разница NPV в BASE и предотвращённый дефицит в STRESS,
  с общими обязательствами до шока и допустимыми поздними решениями.
- **Реакция с 2038:** сохранение ранних контрактов, отдельные дозаказы с датами
  и общей мощностью. Полное или частичное восстановление с остаточными нарушениями.
- **Цена сервиса:** 97%/99% против 100% при одинаковых инвестициях.
- **Пять вариантов:** исходный BASE, шок без действий, реакция в пределах брони,
  условные новые контракты и подготовленная стратегия.
- **Обоснование выбора:** шесть кандидатов, доплата за защиту от общего земного сбоя,
  оставшийся дефицит и реестр оснований. В интерфейсе — вкладка «Почему этот план».
- **Риски:** последствия, владельцы, меры и остаток; матрица чувствительности,
  границы до первого дефицита и отдельный геополитический сценарий.
- **Расширение:** кнопка F/2042, импорт произвольных дополнительных каналов и
  будущих лет через `research_config` (до 12 каналов, 2045). Исходные данные сохранены.
- **Выгрузка:** обычные CSV/XLSX/JSON и полный конкурсный JSON/CSV с протоколом.

Проверка и генерация согласованных материалов после установки зависимостей:

```bash
python tools/build_contest_evidence.py
```

Результаты в [`results/`](results/); [протокол V01–V10 и E01–E32](docs/validation_protocol.md),
[резюме сценариев](docs/scenario_summary.md), [стейкхолдеры и контракты](docs/stakeholders_and_contracts.md),
[геополитика](docs/geopolitical_bonus.md), [план записки](docs/contest_note_outline.md),
[12 слайдов](docs/presentation_outline.md). Последние два файла — подробные
текстовые каркасы для финальной вёрстки, а не готовые PDF/PPTX.

В UI пройдите шаги «Настроить план → Проверить шок → Сравнить стратегии → Сохранить решение». Подробности: [методика реакции](docs/RESPONSE.md). «Открыть план»
переносит выбранную альтернативу в основной редактор. При изменении плана
аналитический отчёт сбрасывается, чтобы не показывать старые числа как новые.
Исследовательские задержки рассчитаны без наверстывания пропущенных поставок;
вероятности и денежная стоимость срыва миссий не выдумываются.
...
Открыть http://localhost:8080. API и интерактивная документация: http://localhost:8000/docs.
Первый запуск устанавливает зависимости и собирает React; следующие используют кэш.

Для разработки установите Python 3.12 и Node.js 22 LTS, затем:
...
Открыть http://localhost:5173. На Linux/macOS заменить `.venv/Scripts/python`
на `.venv/bin/python`. Полная инструкция: [запуск и размещение](docs/DEPLOYMENT.md).

## Что реализует приложение

- Дневной материальный баланс, приоритет критического спроса, физический резерв,
  проверка склада и доступности поставок с lead time.
- Подготовительная закупка начального запаса, годовой take-or-pay, резервирование,
  хранение, CAPEX/OPEX и приведённая стоимость расходов.
- Отдельные BASE и MANDATORY_STRESS, численные нарушения с причиной и периодом.
- LP-оптимизация закупок при выбранных инвестициях с повторной проверкой симулятором.
- Интерактивный граф логистики, динамика запасов и годовая таблица.
- Сохранение и повторная загрузка плана JSON, экспорт CSV/XLSX/JSON через API.

NPV в интерфейсе означает приведённую **стоимость обеспечения**, а не прибыль.
Деньги измеряются в миллионах условных единиц постоянных цен 2035 года.
Начальная стратегия — проверяемый пример; её наличие не доказывает оптимальность.

## Структура решения

| Путь | Назначение |
|---|---|
| `app/core/` | Константы из CSV/YAML, симулятор, LP-оптимизатор |
| `app/schemas/` | Pydantic-схемы входов |
...
[Модель и допущения](docs/MODEL.md) · [Оптимизатор](docs/OPTIMIZER.md) ·
[REST API](docs/API.md) · [Сценарий приёмки](docs/ACCEPTANCE.md)

Проверки из корня, после установки зависимостей:
...
Поставка моделируется равномерно по дням доступности источника, год содержит 365
модельных дней. Резерв физический и проверяется на начало года. Неиспользованная
бронь Emergency не засчитывается как топливо в узле. Оптимизатор фиксирует выбранные
годы инвестиций и начальный запас; оптимальность относится к этим решениям и
принятому расписанию поставок. База аккаунтов, совместное серверное хранение планов
и вероятностная модель реальных отказов в текущую версию не входят.

---

# Исходные материалы организаторов

Ниже сохранено описание стартового набора. Указания об отсутствии приложения
относятся к исходному набору, до реализации решения команды.
